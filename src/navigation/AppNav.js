...
class AppNav extends Component {
  ...
  handleNavigationChange = async () => {
    const userToken = await AsyncStorage.getItem('loggedUser');
    const userStore = await AsyncStorage.getItem('storeUser');

    if (userToken !== userStore) {
      if (userToken) {
        await AsyncStorage.setItem('storeUser', userToken);
      } else {
        await AsyncStorage.removeItem('storeUser');
      }

      const userToken2 = await AsyncStorage.getItem('storeUser');
      if (userToken2) {
        this.setState({ user: JSON.parse(userToken2) });
      } else {
  ...
  render() {
    ...
    const routeName = user ? "Profile" : "Auth";
    const signInScreensWithBottomBarVisible = createStackNavigator(
      ...
      { initialRouteName: routeName, headerMode: 'none' }
    );
    ...
    return (
      <AppNavigation onNavigationStateChange={this.handleNavigationChange} />
    );
  }
}
